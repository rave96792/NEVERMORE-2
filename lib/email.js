  ...
  if (SHOP_TO) {
    try {
      const html = orderHtml({
        title: 'New gang sheet submission',
        subtitle: `Order ${order.id} · ${money(order.total)} · ${order.items?.length || 0} item(s)`,
        order,
      })
      const r = await resend.emails.send({
        from: FROM,
        to: SHOP_TO,
        subject: `NEW ORDER · ${order.items?.[0]?.sheetLabel || 'Gang sheet'} · ${money(order.total)}`,
        html,
      })
  ...
  if (BUYER_CONFIRM && order.shipping?.email) {
    try {
      ...
      const r = await resend.emails.send({
        ...
        subject: `Nevermore DTF · order ${order.id.slice(0, 8)} confirmed`,
        html,
      })
