  ...
  <div style="max-width:640px;margin:0 auto;padding:24px">
    ...
    <div style="background:#fff;color:#111;border-radius:12px;margin-top:16px;overflow:hidden">
      ...
      <div style="padding:12px 20px 4px;font-size:13px;color:#333">
        ...
        <div style="display:flex;justify-content:space-between"><span>${order.deliveryMethod === 'pickup' ? 'Local pickup' : 'Shipping'}</span><span>${order.deliveryMethod === 'pickup' ? 'FREE' : (order.shipping_amount ? money(order.shipping_amount) : 'FREE')}</span></div>
        ${order.rush ? `<div style="display:flex;justify-content:space-between;color:#b45309"><span>Rush production</span><span>${money(order.rushFee || 30)}</span></div>` : ''}
      ...
      <div style="padding:16px 20px;border-top:1px solid #eee">
        ${order.deliveryMethod === 'pickup' ? `
          <div style="font-size:11px;letter-spacing:0.15em;text-transform:uppercase;color:#059669;margin-bottom:6px">Free local pickup</div>
          <div style="font-weight:600">${escapeHtml(s.fullName || '')}</div>
          <div style="color:#555">${escapeHtml(s.email || '')}${s.phone ? ' · ' + escapeHtml(s.phone) : ''}</div>
          <div style="color:#555;margin-top:8px">We'll email you as soon as your gang sheet is ready. Nevermore DTF · Honolulu, HI.</div>
        ` : `
