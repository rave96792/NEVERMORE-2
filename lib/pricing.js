...
// Shipping: HI $5 flat, everywhere else $12 flat.
export const SHIPPING_RATES = {
  hawaii: 5.00,
  standard: 12.00,
}

export function computeShipping({ country, state, deliveryMethod }) {
  if (deliveryMethod === 'pickup') return 0
  const s = String(state || '').toUpperCase().trim()
  if (s === 'HI') return SHIPPING_RATES.hawaii
...
export const RUSH_FEE = 30 // Flat $30 rush production upcharge

export function computeTotals({ subtotal, shippingState, shippingCountry, deliveryMethod, rush }) {
  const shipping = computeShipping({ country: shippingCountry, state: shippingState, deliveryMethod })
  const rushFee = rush ? RUSH_FEE : 0
  const state = String(shippingState || '').toUpperCase().slice(0, 2)
  const rate = TAX_RATES[state] || 0
  // Rush fee is taxable in HI (part of the sale)
  const tax = round2((subtotal + shipping + rushFee) * rate)
  const total = round2(subtotal + shipping + rushFee + tax)
  return {
